...
exclusion_variables <- c("Country", "Payment_Method_Credit_Debit", "Online_Service_Preference", 
...
ai_satisfaction_summary_canada <- data_country_canada %>%
  select(-c(exclusion_variables)) %>%  # Exclude variables
  tbl_summary(
    by = AI_Satisfaction,    # Stratifying by AI_Satisfaction
    statistic = list(all_categorical() ~ "{n} ({p}%)",  # Categorical summary
                     all_continuous() ~ "{mean} ({sd})"), # Continuous summary
    digits = all_continuous() ~ 2  # Round continuous variables to 2 decimal places
  ) %>%
  add_p()


ai_satisfaction_summary_canada %>%
  as_gt() %>%
  tab_header(
    title = "Summary Statistics for Canada"
  )

# Create a stratified summary table of demographic variables by AI_Satisfaction
ai_satisfaction_summary_china <- data_country_china %>%
  select(-c(exclusion_variables)) %>%  # Exclude variables
  tbl_summary(
    by = AI_Satisfaction,    # Stratifying by AI_Satisfaction
    statistic = list(all_categorical() ~ "{n} ({p}%)",  # Categorical summary
...
ai_satisfaction_summary_canada %>%
  as_gt() %>%
  tab_header(
    title = "Summary Statistics for China"
  )

# Create a stratified summary table of demographic variables by AI_Satisfaction
ai_satisfaction_summary_india <- data_country_india %>%
  select(-c(exclusion_variables)) %>%  # Exclude variables
  tbl_summary(
    by = AI_Satisfaction,    # Stratifying by AI_Satisfaction
    statistic = list(all_categorical() ~ "{n} ({p}%)",  # Categorical summary
...
ai_satisfaction_summary_canada %>%
  as_gt() %>%
  tab_header(
    title = "Summary Statistics for India"
  )
```
